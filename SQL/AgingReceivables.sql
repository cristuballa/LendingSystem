Alter PROCEDURE [dbo].[AgingReceivables]
(@DateAsOf datetime)
AS 
BEGIN
...
set @DateAsOf=GetDate()
 ...
 declare @AOR TABLE 
    ...
    INSERT INTO @AOR (DueDate,LoanId,BorrowerId,LoanAmount,InterestRate,TermsOfPayment,AmountDue,Payment,Balance,
		 ...
		 Select b.FirstName+' '+b.LastName as Name,b.Address,b.ContactInfo, a.Balance,a.Payment,a.AmountDue,a.DueDate,a.TermsOfPayment,
		       (isnull((Select (LoanAmount +(LoanAmount*(InterestRate/100)*TermsOfPayment))/TermsOfPayment From Loans Where  LoanId=a.LoanId),0)) as MonthlyDue,
		       CONVERT(date, CAST(DATEPART(month,getdate()) AS char(2)) + '-' + CAST(DATEPART(day,getdate()) AS char(2)) +'-'+ CAST(DATEPART(YEAR ,getdate()) AS char(4)),101) as DueDate1
		         ...
		         ,CASE WHEN isnull(a.AgingNoOfDays,0)=0 THEN NULL 
			      ...
			      ELSE isnull(a.DueToDate,0) END as TotalDue
			      

		 From @AOR a  INNER JOIN Borrowers b ON a.BorrowerId  =b.BorrowerId  WHERE b.Status =1 and isnull(a.balance,0)>0 ORDER BY b.FirstName   
